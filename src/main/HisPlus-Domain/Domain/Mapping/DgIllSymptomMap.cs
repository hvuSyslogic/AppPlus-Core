...
namespace HisPlus.Domain.Mapping
{
    public class DgIllSymptomMap : EntityTypeConfiguration<DgIllSymptom>
    {
        ...
        public DgIllSymptomMap(string schema)
        {
            ToTable("DgIllSymptom", schema);
            HasKey(x => x.Id);

            Property(x => x.Id).HasColumnName(@"ID").IsRequired().HasColumnType("int").HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.Identity);
            Property(x => x.IcdId).HasColumnName(@"IcdId").IsOptional().HasColumnType("int");
            Property(x => x.Symptom).HasColumnName(@"Symptom").IsOptional().HasColumnType("nvarchar").HasMaxLength(50);
