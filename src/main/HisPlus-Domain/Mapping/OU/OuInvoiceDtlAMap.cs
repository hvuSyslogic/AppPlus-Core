...
{
    public class OuInvoiceDtlAMap : EntityTypeConfiguration<OuInvoiceDtlA>
    {
        public OuInvoiceDtlAMap()
            : this("dbo")
        {
        }

        public OuInvoiceDtlAMap(string schema)
        {
            ToTable("OuInvoiceDtl_A", schema);
            HasKey(x => new { x.OperDate, x.Id, x.InvoId, x.ItemId, x.Totality, x.UnitId, x.Price, x.DiscDiag, x.IsModiDisc, x.DiscSelf, x.Amount, x.AmountFact, x.AmountSelf, x.AmountTally, x.LsPerform, x.LimitFee, x.ExecLocId, x.DoctorId, x.InvItemId, x.FeeId, x.Memo });
