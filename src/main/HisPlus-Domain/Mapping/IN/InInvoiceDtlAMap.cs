...
{
    public class InInvoiceDtlAMap : EntityTypeConfiguration<InInvoiceDtlA>
    {
        public InInvoiceDtlAMap()
            : this("dbo")
        {
        }

        public InInvoiceDtlAMap(string schema)
        {
            ToTable("InInvoiceDtl_A", schema);
            HasKey(x => new { x.OperDate, x.Id, x.HospId, x.ItemId, x.RegOperTime, x.RegOperId, x.LocationId, x.AdviceId, x.LsMarkType, x.Totality, x.PriceIn, x.UnitId, x.IsPay, x.DiscIn, x.DiscSelf, x.Amount, x.AmountFact, x.AmountSelf, x.AmountTally, x.AmountPay, x.LsPerform, x.IsModiDisc, x.DoctorId, x.ExecOperId, x.ExecLocId, x.InvItemId, x.FeeId, x.OperTime, x.OperId, x.IsCancel, x.IsManual });
