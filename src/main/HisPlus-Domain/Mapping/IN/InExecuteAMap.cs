...
{
    public class InExecuteAMap : EntityTypeConfiguration<InExecuteA>
    {
        public InExecuteAMap()
            : this("dbo")
        {
        }

        public InExecuteAMap(string schema)
        {
            ToTable("InExecute_A", schema);
            HasKey(x => new { x.OperDate, x.Id, x.OperTime, x.OperId, x.LocationId, x.AdviceId, x.LsMarkType, x.Memo, x.IsPrint });
