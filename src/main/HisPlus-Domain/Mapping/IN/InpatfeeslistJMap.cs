...
{
    public class InpatfeeslistJMap : EntityTypeConfiguration<InpatfeeslistJ>
    {
        public InpatfeeslistJMap()
            : this("dbo")
        {
        }

        public InpatfeeslistJMap(string schema)
        {
            ToTable("INPATFEESLIST_J", schema);
            HasKey(x => new { x.Id, x.HospId, x.InTime, x.PatTypeId, x.Lsinstatus2 });
            ...
            Property(x => x.Lsinstatus2).HasColumnName(@"LSINSTATUS2").IsRequired().IsUnicode(false).HasColumnType("varchar").HasMaxLength(2);
        }
    }
}
