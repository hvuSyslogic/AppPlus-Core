...
namespace HisPlus.Redis
{
    public static partial class CacheProviderEx
    {
        ...
        #endregion
        public static string GetKey<T>(this ICacheProvider cache)
        {
            return string.Format("{0}:{1}", GetEntityName<T>(), "HASH");
        }

        public static string GetKey<T>(this ICacheProvider cache, T value = default(T), Func<T, object> keyBuilder = null)
        {
            if (EqualityComparer<T>.Default.Equals(value, default(T)) || keyBuilder == null)
            {
                return cache.GetKey<T>();
            }

            return string.Format("{0}:{1}", GetEntityName<T>(), keyBuilder.Invoke(value));
        }        

        public static string GetHashField<T>(this ICacheProvider cache, T value, Func<T, object> hashFieldBuilder)
        {
            return string.Format("{0}:{1}", GetEntityName<T>(), hashFieldBuilder.Invoke(value));
        }

        public static string[] GetTags<T>(this ICacheProvider cache, T value, Func<T, string[]> tagsBuilder)
        {
            return tagsBuilder.Invoke(value).Select(x => string.Format("{0}:{1}", GetEntityName<T>(), x)).ToArray();
        }
        ...
        {
            var key = cache.GetKey<T>(value, keyBuilder);
            var tags = cache.GetTags<T>(value, tagsBuilder);
        ...
        public static List<string> GetRawKeysByTag<T>(this ICacheProvider cache, T value, Func<T, string[]> tagsBuilder, bool cleanUp = false)
        {
            var tags = cache.GetTags<T>(value, tagsBuilder);
        ...
        {
            var keys = values.Select(x => cache.GetKey<T>(x, keyBuilder)).ToArray();
            cache.Remove(keys);
        }
        ...
        {
            var key = cache.GetKey<T>();
            var field = cache.GetHashField<T>(value, hashFieldBuilder);
            var tags = cache.GetTags<T>(value, tagsBuilder);
        ...
        {
            var key = cache.GetKey<T>();
            var fieldValues = values.ToDictionary(x => cache.GetHashField<T>(x, hashFieldBuilder));
        ...
        {
            var key = cache.GetKey<T>();
            var field = cache.GetHashField<T>(value, fieldBuilder);
            var tags = cache.GetTags<T>(value, tagsBuilder);
